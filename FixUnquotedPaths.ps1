$ComputerList = Get-ADComputer -Filter 'OperatingSystem -like "*Win*"'  | Select-Object Name
$ComputerList = $ComputerList.Name
$count = 0
				...
				if ($si.ImagePath -ne $nul) { 
					...
					if ($badpath -eq $true){
						...
						if ($obj.badkey -eq "Yes"){
							$ServiceName =  $obj.key.Split("\")[-1]
							$OldPath = $obj.ImagePath
							$NewPath = $obj.Fixedkey
							#$obj
							Write-Host "Service: $ServiceName"
							Write-Host "OLD Path: "
							Write-Host $OldPath -ForegroundColor Red
							Write-Host "NEW Path: "
							Write-Host $newpath -ForegroundColor Green
							Write-Host
							$proceed = Read-Host -Prompt "Proceed to fix? [y/n]" 
							if ($proceed -eq "y") {
